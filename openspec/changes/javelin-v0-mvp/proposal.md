...
- **New: LLM provider abstraction** — Backend integration with a cost-efficient LLM (OpenRouter (primary) / OpenAI (secondary)) via an abstracted provider layer, enabling model swaps without rewriting business logic. Declared in `stripe-app.json` CSP.
...
- **LLM provider API** (OpenRouter (primary) / OpenAI (secondary)) — must be declared in `stripe-app.json` CSP
