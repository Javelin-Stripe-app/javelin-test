...
> **Note:** Feedback UI is embedded in `InsightCard` (`src/components/InsightCard.tsx`). There is no standalone `InsightFeedback` component. State management for feedback is co-located with the card.

#### Scenario: Thumbs up/down on insight
- **WHEN** an insight is displayed via the `InsightCard` component
- **THEN** `InsightCard` SHALL render thumbs up and thumbs down `Button` components inline below the `Banner`
- **AND** tapping a button SHALL send a `POST /insights-feedback` request via `apiClient`
- **AND** the button state SHALL update to show the selected feedback (disabled after selection)
