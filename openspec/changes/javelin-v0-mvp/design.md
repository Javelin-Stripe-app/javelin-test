...
**Token storage (implementation update):** The original design specified Stripe Secret Store as the sole token store. During implementation, Secret Store retrieval proved unreliable from Edge Functions, so an `oauth_tokens` table was added to Supabase Postgres as the primary token store. The Secret Store remains as a secondary mechanism. All token access is RLS-scoped. Additionally, PKCE `code_verifier` was required for the OAuth code exchange — this wasn't anticipated in the original design.
...
- Secret Store only: Prescribed by Stripe, but proved unreliable for Edge Function retrieval. Database storage provides reliable, queryable token access with RLS isolation.
...
**Choice:** An `LLMProvider` interface in the Edge Functions layer that defines `generateResponse(prompt, context)` and `generateInsights(data)`. Implementation routes through OpenRouter (primary) with OpenAI as a secondary provider. Both are declared in `stripe-app.json` CSP (`connect-src`).

**Resolved (v0.1):** LLM provider selection — OpenRouter selected as primary gateway for model flexibility and cost efficiency. OpenAI API also whitelisted for fallback/direct access.
...
**CSP declaration:** The LLM provider URLs are declared in `stripe-app.json` `content_security_policy.connect-src`: `https://openrouter.ai/api/` and `https://api.openai.com/v1/`.
...
|--------|------|---------|------|
| POST | `/auth-callback` | Exchange OAuth code for tokens, store in DB + Secret Store | Stripe signature |
...
| POST | `/sync-data` | Upsert synced Stripe records into cache tables | Stripe signature |
| POST | `/sync-worker` | Background sync job processor (paginate Stripe API, call sync-data) | Service role |
...
| GET | `/chat-conversation-detail` | Get messages for a conversation | Stripe signature |
| GET | `/metrics/:type` | Unified metrics endpoint — routes to MRR, churn, ARPU, LTV, or revenue by URL path | Stripe signature |
...
| POST | `/data-lifecycle` | Purge cached data not synced in 60 days | Service role |

> **Implementation note:** Supabase Edge Functions use kebab-case names (one function per directory), not nested REST paths. The original `/auth/callback` design became `auth-callback`, etc. Metrics are handled by a single `metrics` function that routes internally by URL path segment (`/metrics/mrr`, `/metrics/churn`, etc.).
...
Migration 1 (`20260212095537_initial_schema.sql`) — creates all 12 core tables:
...
Migration 2 (`20260212155052_add_set_app_context_rpc.sql`) — adds `set_app_context(account_id TEXT)` RPC function (`SECURITY DEFINER`). Called by Edge Function auth middleware to set `app.stripe_account_id` config before DB queries. This is how RLS context is established — Edge Functions call `SELECT set_app_context('acct_xxx')` rather than setting the config directly, ensuring consistent context propagation.

Migration 3 (`20260213100458_add_oauth_tokens.sql`) — adds `oauth_tokens` table for per-account Stripe API access. Stores access + refresh tokens from the OAuth 2.0 flow directly in Postgres (in addition to the Secret Store). This was added during implementation when the Secret Store proved unreliable for token retrieval in Edge Functions — the database serves as the primary token store, with the Secret Store as a secondary mechanism.

**All tables include:**
- `stripe_account_id` column for RLS scoping
- RLS policy: `stripe_account_id = current_setting('app.stripe_account_id', true)`
- `created_at` / `updated_at` timestamps (auto-updated via `update_updated_at()` trigger)
...
|-----------|---------------|---------|-----------|
| Stripe OAuth tokens | Secret | Supabase Postgres (`oauth_tokens`, RLS-enforced) + Stripe Secret Store (secondary) | Until refresh expiry (1yr rolling) |
...
1. **LLM provider selection:** ~~Kimi K2, DeepSeek, or ChatGPT?~~ **Resolved:** OpenRouter selected as primary LLM gateway (model flexibility + cost efficiency). OpenAI API also whitelisted as secondary/fallback. Both declared in `stripe-app.json` CSP.
2. **Stripe Benchmarking API access:** ~~Need to verify API availability.~~ **Partially resolved:** `benchmarking-compare` Edge Function implemented. API access status unclear — function exists but programmatic API availability not confirmed. Fallback to account-only metrics implemented (graceful degradation on API unavailability).
3. **Sync trigger mechanism:** ~~Auto on install or on first chat?~~ **Resolved:** Auto-sync triggers on app load. The ChatView proactively checks auth status and triggers sync on mount, regardless of conversation state. Sync also runs when data staleness is detected (>1 hour since last sync) before answering chat questions.
4. **Conversation title generation:** ~~LLM-generated or timestamp-based?~~ **Resolved:** Auto-generated from first message content.

## Open Questions

1. **OAuth token storage strategy:** Implementation evolved to store tokens in both Supabase Postgres (`oauth_tokens` table) and Stripe Secret Store. The database is the primary store due to reliability issues with Secret Store retrieval in Edge Functions. Should Secret Store be kept as secondary, or fully deprecated in favor of DB-only? PKCE `code_verifier` parameter was added to fix the OAuth code exchange flow.
2. **Distribution type:** Currently `"private"` (invite-only beta). When should this transition to `"public"` for Stripe App Marketplace listing?
