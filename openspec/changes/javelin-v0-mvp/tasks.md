...
- [x] 1.1 Initialize Stripe App project with `stripe apps create` — configure `stripe-app.json` manifest with `distribution_type: "private"` (invite-only beta), `stripe_api_access_type: "oauth"`, sandbox install compatible
...
- [x] 2.13 Create RLS policies for ALL tables: `stripe_account_id = current_setting('app.stripe_account_id', true)` — verify no table is accessible without RLS
- [x] 2.14 Create `set_app_context(account_id TEXT)` RPC function (`SECURITY DEFINER`) — called by Edge Function auth middleware to set RLS context before DB queries
- [x] 2.15 Create `oauth_tokens` table — per-account Stripe OAuth token storage (primary store, Secret Store as secondary) with RLS
- [x] 2.16 Run migrations against Supabase development instance and verify schema

## 3. Authentication (stripe-auth)

- [x] 3.1 Implement Edge Function: `POST /auth-callback` — accept OAuth code + state + PKCE code_verifier, exchange for access/refresh tokens via Stripe OAuth token endpoint, store in `oauth_tokens` table (primary) and Secret Store (secondary), return account ID
- [x] 3.2 Implement Edge Function: `POST /auth-refresh` — retrieve refresh token from `oauth_tokens` table, exchange for new access token, update both DB and Secret Store, return new expiry
...
- [x] 3.4 Implement shared auth middleware: `setRLSContext()` — call `set_app_context(account_id)` RPC to set `app.stripe_account_id` Postgres setting before any DB query; chain with signature verification
- [x] 3.5 Implement shared auth middleware: `getStripeClient()` — retrieve OAuth access token from `oauth_tokens` table (fallback: Secret Store), auto-refresh if expired, return authenticated Stripe SDK client
...
- [x] 4.7 Implement Edge Function: `POST /sync-data` — accept synced Stripe records and upsert into cache tables with extracted indexed columns
- [x] 4.8 Implement Edge Function: `POST /sync-worker` — background job processor that paginates Stripe API and calls sync-data for batch upserts
- [x] 4.9 Implement Edge Function: `POST /data-lifecycle` — purge cached rows not synced in 60 days across all 6 cache tables
- [x] 4.10 Integrate uninstall hook: wire `DELETE /account-data` to purge all cache tables for the account
- [x] 4.11 Create Zod schemas for sync endpoint request/response validation
...
- [x] 5.6 Implement unified Edge Function: `GET /metrics/:type` — single `metrics` function that routes by URL path segment (mrr, churn, arpu, ltv, revenue); parse query params with Zod per metric type, call corresponding calculation function, return with freshness envelope
- [x] 5.7 Pre-load Stripe billing metric definitions as constants — align all calculation logic with Stripe's documented definitions
...
- [x] 6.2 Implement concrete LLM provider adapter — OpenRouter (primary) + OpenAI (secondary); HTTP calls to provider API, parse response, track token usage
...
- [x] 12.2 Verify all 16 Edge Function endpoints reject requests without valid Stripe signatures (401)
