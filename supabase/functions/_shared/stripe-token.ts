...
export async function getStripeAccessToken(
  ...
  if (error || !tokenRow) {
    // In dev/private mode, the platform secret key IS the installed account's key.
    if (DEV_MODE && STRIPE_SECRET_KEY) {
      console.warn(
        `[stripe-token] DEV_MODE fallback: Using STRIPE_SECRET_KEY for account ${stripeAccountId}. ` +
        'This is expected for private/sandbox apps where OAuth redirect does not occur.',
      );
